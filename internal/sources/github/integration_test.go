...
func setupCollector(t *testing.T, cfg domainCollectorConfig, fake *fakeTransport) *Collector {
	t.Helper()
	if fake == nil {
...
func TestCollect_MixedIssuesAndDiscussions(t *testing.T) {
	...
	c := setupCollector(t, domainCollectorConfig{
		Enabled:            true,
		SearchIssues:       true,
...
func TestCollect_NilContext(t *testing.T) {
	c, err := New(domainCollectorConfig{Enabled: true, MaxRequests: 100})
	if err != nil {
...
func TestCollect_NotEnabled(t *testing.T) {
	_, err := New(domainCollectorConfig{Enabled: false})
